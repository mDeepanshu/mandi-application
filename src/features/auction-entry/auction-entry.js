...
function AuctionEntries() {
  ...
  const getAuctionEntries = async () => {
    ...
    if (vasuliList) {
      ...
      const itemWiseTotals = getItemWiseQuantity(vasuliList.responseBody);
  ...
  useEffect(() => {
    if (tableDataFiltered && tableDataFiltered.length > 0) setCheckedEntries(Array(tableDataFiltered?.length).fill(false));
  }, [tableDataFiltered]);
  ...
  const getItemWiseQuantity = (data) => {
    const resultMap = {};
